...
export default class ServiceQuestionService {
  ...
  findServiceQuestionById = id =>
    fetch(`${API_ROOT}/api/service-questions/${id}`).then(response =>
      response.json()
    );

  findAllServiceQuestions = () =>
    fetch(`${API_ROOT}/api/service-questions`).then(response =>
      response.json()
    );

  deleteServiceQuestion = id =>
    fetch(`${API_ROOT}/api/service-questions/${id}`, { method: "DELETE" }).then(
